  ...
  )
{
  ...
  return memcmp (DataDigest, Hash, HashSize);
}
  ...
  )
{
  ...
  if (SignatureSize != ModulusSize) {
    fprintf(stderr, "OCCR: Signature length does not match key length\n"); //FIXME
  ...
  CmpResult = memcmp (&Signature[Index], Padding, PaddingSize);
  if (CmpResult != 0) {
    return FALSE;
  }
  ...
  CmpResult = memcmp (&Signature[Index], Hash, HashSize);
  if (CmpResult != 0) {
    return FALSE;
  }
