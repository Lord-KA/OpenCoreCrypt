  ...
  )
{
  ...
  //
  memcpy (Context->State, SHA512_H0, SHA512_DIGEST_SIZE);
  ...
  )
{
  ...
  memcpy (&Context->Block[Context->Length], Data, RemLen);
  ...
  memcpy (Context->Block, &ShiftedMsg[BlockNb << 7], RemLen);
...
{
  memcpy (Context->State, SHA384_H0, SHA512_DIGEST_SIZE);
